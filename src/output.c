...
FILE *f;
  if (jflag)/*rwj*/
    ...
    fprintf(code_file,"    val = val.clone();\n");
    fprintf(code_file,"  } catch (CloneNotSupportedException e1) {\n");
    fprintf(code_file,"    // only clone if it is supported\n");
    fprintf(code_file,"  }\n");
    fprintf(code_file,"  try {\n");
    ...
    fprintf(code_file,"final %s dup_yyval(%s val)\n",jvalclass,jvalclass);
    fprintf(code_file,"{\n");
    fprintf(code_file,"  %s dup = new %s(val);\n",jvalclass,jvalclass);
